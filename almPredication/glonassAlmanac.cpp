...
void GlonassAlmanac::predicationSatellitePosition(const Time& curTime, Satellite& satellite) const 
{
	...
	unsigned int jd = data_to_julian(curTime.get_day(), curTime.get_month(), curTime.get_year());
	jd = (jd % 1461);
	/* The current time in sec */
	double sec = (curTime.get_hour() - 0) * 3600 + curTime.get_min() * 60 + curTime.get_sec();

	int prn = satellite.get_prn();
	const SatelliteGlonassAlmanac*	oneGlnSatAlmanac = &almanac[prn - 1];
	...
	satellite.coord.xs = x1o*e0x1 + x2o*e0x2;
	satellite.coord.ys = x1o*e0y1 + x2o*e0y2;
	satellite.coord.zs = x1o*e0z1 + x2o*e0z2;
	satellite.coord.vflg = 1;
	satellite.vel.vxs = xo1*e0x1 + xo2*e0x2 + PZ90_OE*satellite.coord.ys;
	satellite.vel.vys = xo1*e0y1 + xo2*e0y2 - PZ90_OE*satellite.coord.xs;
	satellite.vel.vzs = xo1*e0z1 + xo2*e0z2;
	satellite.vel.vflg = 1;

}

void ReadGlnAlmanac::readAlmanac(const std::string& fileName, Almanac* almanac)
{
